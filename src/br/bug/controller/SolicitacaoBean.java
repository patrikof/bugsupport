...
import br.bug.service.SolicitacaoService;
...
public class SolicitacaoBean{
	...
	@Autowired
	private SolicitacaoService gerenciador;
	@Autowired
	private ClienteService gerenciadorCliente;
	@Autowired
	private ProdutoService gerenciadorProduto;
	...
	public String cadastrar() {
		limparDados();
		return FacesUtil.redirect("/view/solicitacao/form.jsf");
	}
	...
	public String salvar() {
		try {
			...
			if (cadastroNovo) {
				...
				return FacesUtil.forward("/view/solicitacao/lista.jsf");
				/*NavigationHandler nh = FacesUtil.getFacesContext().getApplication().getNavigationHandler();  
				nh.handleNavigation(FacesUtil.getFacesContext(), null, "/view/solicitacao/lista.jsf");
				return null;*/
	...
	public String alterar(int id) {
		...
		if ((atendente == usuarioLogado) || (responsavel == usuarioLogado)) {
			...
			return FacesUtil.redirect("/view/solicitacao/form");
		} else {
			FacesUtil
	...
	public String alterarResponsavel(int id) {
		...
		solicitacao.setResponsavel(new Usuario());
		return FacesUtil.redirect("/admin/alteraResponsavel");
	}
